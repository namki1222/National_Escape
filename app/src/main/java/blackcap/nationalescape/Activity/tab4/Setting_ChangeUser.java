    ...
    public void setSave(){
        Txt_Save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(Edit_Nickname.getText().toString().equals(Str_Nickname) && Edit_Pass.getText().toString().equals(Str_Pass)){
                    Toast.makeText(Setting_ChangeUser.this, "변경된 정보가 없습니다", Toast.LENGTH_SHORT).show();
                }
                else{
                    if(Edit_Pass.getText().toString().length() < 8){
                        Toast.makeText(Setting_ChangeUser.this, "비밀번호를 8자리 이상 입력해주세요", Toast.LENGTH_SHORT).show();
                    }
                    else{
    ...
    public class Async_Update_Nick extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if(result_nick.equals("exist")){
                ...
                Async_Update_Pass async_update_pass = new Async_Update_Pass();
                async_update_pass.execute();
            }
            else{
    ...
    public class Async_Update_Pass extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                //유저 정보 response
                HttpClient http = new HttpClient();
                result_pass = http.HttpClient("Web_Escape", "User_Modify_Pass.jsp",User_Pk, Edit_Pass.getText().toString());
